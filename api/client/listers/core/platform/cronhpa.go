...
// List lists all CronHPAs in the indexer.
func (s *cronHPALister) List(selector labels.Selector) (ret []*platform.CronHPA, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.CronHPA))
	})
	return ret, err
}

// Get retrieves the CronHPA from the index for a given name.
func (s *cronHPALister) Get(name string) (*platform.CronHPA, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("cronhpa"), name)
	}
	return obj.(*platform.CronHPA), nil
}
