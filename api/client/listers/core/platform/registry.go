...
type RegistryLister interface {
	...
	List(selector labels.Selector) (ret []*platform.Registry, err error)
	// Get retrieves the Registry from the index for a given name.
	Get(name string) (*platform.Registry, error)
...
// List lists all Registries in the indexer.
func (s *registryLister) List(selector labels.Selector) (ret []*platform.Registry, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.Registry))
	})
	return ret, err
}

// Get retrieves the Registry from the index for a given name.
func (s *registryLister) Get(name string) (*platform.Registry, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("registry"), name)
	}
	return obj.(*platform.Registry), nil
}
