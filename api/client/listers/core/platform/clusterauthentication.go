...
type ClusterAuthenticationLister interface {
	...
	List(selector labels.Selector) (ret []*platform.ClusterAuthentication, err error)
...
// List lists all ClusterAuthentications in the indexer.
func (s *clusterAuthenticationLister) List(selector labels.Selector) (ret []*platform.ClusterAuthentication, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.ClusterAuthentication))
	})
	return ret, err
}
...
type ClusterAuthenticationNamespaceLister interface {
	...
	Get(name string) (*platform.ClusterAuthentication, error)
...
// List lists all ClusterAuthentications in the indexer for a given namespace.
func (s clusterAuthenticationNamespaceLister) List(selector labels.Selector) (ret []*platform.ClusterAuthentication, err error) {
...
// Get retrieves the ClusterAuthentication from the indexer for a given namespace and name.
func (s clusterAuthenticationNamespaceLister) Get(name string) (*platform.ClusterAuthentication, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("clusterauthentication"), name)
	}
	return obj.(*platform.ClusterAuthentication), nil
}
