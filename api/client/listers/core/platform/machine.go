...
type MachineLister interface {
	...
	List(selector labels.Selector) (ret []*platform.Machine, err error)
	// Get retrieves the Machine from the index for a given name.
	Get(name string) (*platform.Machine, error)
...
// List lists all Machines in the indexer.
func (s *machineLister) List(selector labels.Selector) (ret []*platform.Machine, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*platform.Machine))
	})
	return ret, err
}

// Get retrieves the Machine from the index for a given name.
func (s *machineLister) Get(name string) (*platform.Machine, error) {
	obj, exists, err := s.indexer.GetByKey(name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(platform.Resource("machine"), name)
	}
	return obj.(*platform.Machine), nil
}
