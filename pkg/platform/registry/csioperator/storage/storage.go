...
// Storage includes storage for LogCollector and all sub resources.
type Storage struct {
	CSIOperator *REST
	Status      *StatusREST
}
...
func NewStorage(optsGetter genericregistry.RESTOptionsGetter, privilegedUsername string) *Storage {
	...
	store := &registry.Store{
		...
		DefaultQualifiedResource: platform.Resource("csioperators"),
		PredicateFunc:            csioperator.MatchCSIOperator,
	...
	if err := store.CompleteWithOptions(options); err != nil {
		log.Panic("Failed to create CSIOperator etcd rest storage", log.Err(err))
	}

	statusStore := *store
	statusStore.UpdateStrategy = csioperator.NewStatusStrategy(strategy)
	statusStore.ExportStrategy = csioperator.NewStatusStrategy(strategy)

	return &Storage{
		CSIOperator: &REST{store, privilegedUsername},
		Status:      &StatusREST{&statusStore},
	}
}

// ValidateGetObjectAndTenantID validate name and tenantID, if success return CSIOperator
func ValidateGetObjectAndTenantID(ctx context.Context, store *registry.Store, name string, options *metav1.GetOptions) (runtime.Object, error) {
	...
	co := obj.(*platform.CSIOperator)
	if err := util.FilterCSIOperator(ctx, co); err != nil {
		return nil, err
	}
	return co, nil
}

// ValidateExportObjectAndTenantID validate name and tenantID, if success return CSIOperator
...
// REST implements a RESTStorage for LogCollector against etcd.
type REST struct {
	*registry.Store
